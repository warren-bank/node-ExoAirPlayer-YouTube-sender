...
const get_videos_in_range = function(video_ids, range) {
  ...
  let   stop_index    = matches[3] ? parseInt(matches[3], 10) : video_count

  if ((start_index < 0) || (start_index >= video_count))
    start_index = 0

  if ((stop_index <= start_index) || (stop_index > video_count))
    stop_index = video_count

  const video_range_ids = video_ids.slice(start_index, stop_index)
