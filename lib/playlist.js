...
const get_videos_in_range = function(video_ids, range) {
  ...
  const matches = range_spec.exec(range)
  if (!matches)
    return video_ids

  const video_count   = video_ids.length
  const reverse_order = (matches[1] === '-')
  let   start_index   = parseInt(matches[2], 10)
  let   stop_index    = matches[3] ? parseInt(matches[3], 10) : (video_count + 1)

  if (isNaN(start_index) || (start_index < 1) || (start_index > video_count))
    start_index = 1

  if (isNaN(stop_index) || (stop_index <= start_index) || (stop_index > (video_count + 1)))
    stop_index = (video_count + 1)

  const video_range_ids = video_ids.slice((start_index - 1), (stop_index - 1))

  if (reverse_order)
    video_range_ids.reverse()

  verbose.log('Videos in playlist range:', {range, video_count, reverse_order, start_index, stop_index, video_range_ids})
