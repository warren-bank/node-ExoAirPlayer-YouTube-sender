...
const cast_video = function(host, port, video_urls, queue, start_at) {
  if (!host)
  ...
  let POST_data = video_urls.map(video_url => `Content-Location: ${video_url}`).join("\n")

  if (start_at)
    POST_data += "\n" + `Start-Position: ${start_at}`
