...
const process_cli = async function(argv_vals) {
  ...
  const urls = (playlist_id)
    ? await playlist.get_videos_in_playlist(playlist_id).then(video_ids => video_ids.map(video_id => `https://www.youtube.com/watch?v=${video_id}`))
    : [argv_vals["--url"]]
  ...
  for (let i=0; i < videos.length; i++) {
    ...
    await networking.cast_video(device.host, device.port, videos[i].url, queue)
    await networking.delay(5000)
